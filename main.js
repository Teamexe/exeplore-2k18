var snake = document.querySelector('#snake');
var s = document.querySelector('#s');
var math = document.querySelector('#math');
var m = document.querySelector('#m');
var breakout = document.querySelector('#breakout');
var br = document.querySelector('#br');
var tetris = document.querySelector('#tetris');
var tet = document.querySelector('#tet');
var hurdle = document.querySelector('#hurdle');
var hc = document.querySelector('#hc');
var cross = document.querySelector('#cross');
var c = document.querySelector('#c');
var tower = document.querySelector('#tower');
var t = document.querySelector('#t');
var bob = document.querySelector('#bob');
var b = document.querySelector('#b');
var hetrix = document.querySelector('#hetrix');
var h = document.querySelector('#h');



snake.addEventListener('click', function() {
	window.document.location.href = 'Snake/snake.html';
});
s.addEventListener('click', function() {
	window.document.location.href = 'Snake/snake.html';
});

math.addEventListener('click', function() {
	window.document.location.href = '2048/index.html';
});
m.addEventListener('click', function() {
	window.document.location.href = '2048/index.html';
});

breakout.addEventListener('click', function() {
	window.document.location.href = 'breakout/level1.html';
});
br.addEventListener('click', function() {
	window.document.location.href = 'breakout/level1.html';
});

tetris.addEventListener('click', function() {
	window.document.location.href = 'tetris/index.html';
});
tet.addEventListener('click', function() {
	window.document.location.href = 'tetris/index.html';
});

hurdle.addEventListener('click', function() {
	window.document.location.href = 'bat/mine.html';
});
hc.addEventListener('click', function() {
	window.document.location.href = 'bat/mine.html';
});

cross.addEventListener('click', function() {
	window.document.location.href = 'cross/level1.html';
});
c.addEventListener('click', function() {
	window.document.location.href = 'cross/level1.html';
});

tower.addEventListener('click', function() {
	window.document.location.href = 'towers-of-hanoi-master/level1.html';
});
t.addEventListener('click', function() {
	window.document.location.href = 'towers-of-hanoi-master/level1.html';
});

bob.addEventListener('click', function() {
	window.document.location.href = 'bob_the_builder/index.html';
});
b.addEventListener('click', function() {
	window.document.location.href = 'bob_the_builder/index.html';
});

hetrix.addEventListener('click', function() {
	window.document.location.href = 'hextris/index.html';
});
h.addEventListener('click', function() {
	window.document.location.href = 'hextris/index.html';
});
